    ...
    <>
      ...
      <div className={`fixed right-0 top-0 h-full w-full sm:w-[480px] bg-white z-50 shadow-2xl overflow-y-auto transform transition-all duration-300 ease-out ${
        ...
        <div className="sticky top-0 bg-white border-b border-neutral-300 px-6 py-4 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-lg text-neutral-700 hover:bg-neutral-100 hover:text-neutral-700 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-6 py-8">
          ...
          {mode === 'signup' && (
            <div className="bg-gradient-to-br from-indigo-950/40 to-purple-950/40 rounded-xl border border-indigo-800/50 p-5 mb-6">
              <h3 className="font-semibold text-neutral-900 mb-4 text-sm">Why Create an Account?</h3>
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <Star className="w-4 h-4 text-indigo-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm text-neutral-900 font-medium">Follow Your Favorite Creators</p>
                    <p className="text-xs text-neutral-700">Never miss updates from the creators you love</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <BarChart3 className="w-4 h-4 text-indigo-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm text-neutral-900 font-medium">Track Growth in Your Dashboard</p>
                    <p className="text-xs text-neutral-700">See daily stats and growth trends in one place</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <Scale className="w-4 h-4 text-indigo-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm text-neutral-900 font-medium">Save Comparisons</p>
                    <p className="text-xs text-neutral-700">Compare creators side-by-side and save them to your dashboard</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <Clock className="w-4 h-4 text-indigo-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm text-neutral-900 font-medium">View Your Recently Browsed Profiles</p>
                    <p className="text-xs text-neutral-700">Quick access to creators you've checked out</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {mode !== 'reset' && (
            <>
              <button
                ...
                disabled={loading}
                className="w-full bg-white text-neutral-900 py-3 rounded-lg font-medium border-2 border-neutral-300 hover:border-neutral-300 hover:bg-neutral-50 transition-colors flex items-center justify-center gap-2 mb-6 disabled:opacity-50"
              >
              ...
              <div className="relative mb-6">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-white text-neutral-700">Or continue with email</span>
                </div>
              </div>
            </>
          )}
          ...
          <form onSubmit={mode === 'reset' ? handleResetPassword : handleEmailAuth} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-neutral-700" />
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full bg-white border border-neutral-300 rounded-lg py-3 px-10 text-neutral-900 placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 focus:border-transparent"
            ...
            {error && (
              <div className="bg-red-50 border border-red-800 text-red-400 px-4 py-3 rounded-lg text-sm">
                {error}
              </div>
            )}
          ...
          {mode === 'signin' && (
            <div className="mt-4 text-center">
              <button
                ...
                className="text-neutral-700 hover:text-neutral-700 text-sm"
              >
